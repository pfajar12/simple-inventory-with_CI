                ...
                <div class="table-responsive">
                    ...
                    <div class="mb-5">
                        <ul class="nav nav-tabs nav-justified">
                            <li class="nav-item">
                                <a class="nav-link active" data-toggle="tab" href="#panel1" role="tab">Publisher</a>
                            </li>
                            <li class="nav-item">
                                <a class="nav-link" data-toggle="tab" href="#panel2" role="tab">Non-active Publisher</a>
                            </li>
                        </ul>
                    </div>
                            ...
                            <table class="table table-hover table-striped">
                                ...
                                ?>
                                    <tr>
                                        <td><?= $data->name; ?></td>
                                        <td>
                                            <a href="<?= base_url('publisher/edit/'.$data->id);?>">
                ...
                <div class="modal-content">
                    ...
                    <div class="modal-body">
                        <div class="form-group">
                            ...
                            <span class="text-danger font-weight-bold d-none" id="publishername-error">
                                *publisher name must not empty
                            </span>
                        </div>
                    </div>
    ...
    <script type="text/javascript">
        ...
        function checkInput() {
            ...
            if(publishername==''){
                ...
                saveNewData(publishername);
            }
        }

        function saveNewData(publishername) {
            $.ajax({
        ...
        function createNewData(publishername) {
            $.ajax({
                url: '<?= base_url('publisher/create') ?>',
                type: 'POST',
                data: {publishername: publishername},
                beforeSend: function(){
                },
        ...
        function restoredata(id) {
            $.ajax({
                url: '<?= base_url('publisher/restore') ?>',
                type: 'POST',
                async: true,
                data : {id: id},
                beforeSend: function(){
        ...
        function reloadActiveData() {
            $.ajax({
                url: '<?= base_url('publisher/get_data') ?>',
                type: 'GET',
                async: true,
                dataType : 'json',
                beforeSend: function(){
                        ...
                        for(var i=0; i<data.length; i++){
                            $('#tbody-item-get').append('\
                                <tr>\
                                    <td>'+data[i].name+'</td>\
                                    <td>\
                                        <a href="<?= base_url("publisher/edit/'+data[i].id+'");?>">\
                                            <button class="btn btn-warning text-white btn-sm">Edit</button>\
                                        </a>\
        ...
        function reloadNonActiveData() {
            $.ajax({
                url: '<?= base_url('publisher/get_data_nonactive') ?>',
                type: 'GET',
                async: true,
                dataType : 'json',
                beforeSend: function(){
